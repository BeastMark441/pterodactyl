...
@section('title')
    Nests &rarr; Egg: {{ $egg->name }} &rarr; Скрипт установки
@endsection

@section('content-header')
    <h1>{{ $egg->name }}<small>Управление скриптом установки для этого Egg.</small></h1>
    <ol class="breadcrumb">
                ...
                @if(! is_null($egg->copyFrom))
                    <div class="box-body">
                        <div class="callout callout-warning no-margin">
                            Эта service option копирует установочные скрипты и параметры контейнера из <a href="{{ route('admin.nests.egg.view', $egg->copyFrom->id) }}">{{ $egg->copyFrom->name }}</a>. Любые изменения, внесённые вами в этот скрипт, не будут применяться, пока вы не выберете «Нет» в выпадающем списке ниже.
                        </div>
                    </div>
                @endif
                ...
                <div class="box-body">
                    <div class="row">
                        <div class="form-group col-sm-4">
                            ...
                            <p class="text-muted small">Если выбрано значение, скрипт выше будет проигнорирован, а вместо него будет использоваться скрипт из выбранной опции.</p>
                        </div>
                        <div class="form-group col-sm-4">
                            ...
                            <p class="text-muted small">Docker-контейнер, который будет использоваться для запуска этого скрипта для сервера.</p>
                        </div>
                        <div class="form-group col-sm-4">
                            <label class="control-label">Команда entrypoint для скрипта</label>
                            <input type="text" name="script_entry" class="form-control" value="{{ $egg->script_entry }}" />
                            <p class="text-muted small">Команда entrypoint, используемая для этого скрипта.</p>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-12 text-muted">
                            Следующие service options зависят от этого скрипта:
