...
@section('content-header')
    <h1>Хосты баз данных<small>Хосты, на которых могут быть созданы базы данных для серверов.</small></h1>
    <ol class="breadcrumb">
        ...
        <div class="modal-content">
            <form action="{{ route('admin.databases') }}" method="POST">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Создать новый хост баз данных</h4>
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <label for="pName" class="form-label">Имя</label>
                        <input type="text" name="name" id="pName" class="form-control" />
                        <p class="text-muted small">Короткий идентификатор для отличия этого хоста от других. От 1 до 60 символов, например <code>us.nyc.lvl3</code>.</p>
                    </div>
                    <div class="row">
                        <div class="col-md-6">
                            <label for="pHost" class="form-label">Хост</label>
                            <input type="text" name="host" id="pHost" class="form-control" />
                            <p class="text-muted small">IP-адрес или FQDN, который будет использоваться при попытке подключения к этому хосту MySQL <em>из панели</em> для добавления новых баз данных.</p>
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <p class="text-muted small">Пароль для указанной учетной записи.</p>
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="pNodeId" class="form-label">Привязанный узел</label>
                        ...
                        <p class="text-muted small">Эта настройка ничего не делает, кроме как устанавливает этот хост баз данных по умолчанию при добавлении базы данных к серверу на выбранном узле.</p>
                    </div>
                </div>
                <div class="modal-footer">
                    <p class="text-danger small text-left">Учетная запись, указанная для этого хоста баз данных, <strong>должна</strong> иметь разрешение <code>WITH GRANT OPTION</code>. Если учетная запись не имеет этого разрешения, запросы на создание баз данных <em>будут</em> неудачными. <strong>Не используйте те же данные учетной записи для MySQL, которые вы определили для этой панели.</strong></p>
                    {!! csrf_field() !!}
