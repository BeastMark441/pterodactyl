...
@section('title')
    Сервер — {{ $server->name }}: Mounts
@endsection

@section('content-header')
    <h1>{{ $server->name }}<small>Управление mount-точками сервера.</small></h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('admin.index') }}">Админ</a></li>
        <li><a href="{{ route('admin.servers') }}">Сервера</a></li>
        <li><a href="{{ route('admin.servers.view', $server->id) }}">{{ $server->name }}</a></li>
        <li class="active">Монтирования</li>
    </ol>
@endsection
        ...
        <div class="col-sm-12">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">Доступные mounts</h3>
                </div>
                            ...
                            <tr>
                                ...
                                @if (! in_array($mount->id, $server->mounts->pluck('id')->toArray()))
                                    <td class="col-sm-2 middle">
                                        <span class="label label-primary">Не подключён</span>
                                    </td>
                                ...
                                @else
                                    <td class="col-sm-2 middle">
                                        <span class="label label-success">Подключён</span>
                                    </td>
