                        ...
                        <table class="table table-hover">
                            ...
                            <tr>
                                ...
                                @if(is_null($server->external_id))
                                    <td><span class="label label-default">Не задан</span></td>
                                @else
                                    <td><code>{{ $server->external_id }}</code></td>
                                @endif
                            </tr>
                            <tr>
                                <td>UUID / ID Docker-контейнера</td>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    /
                                    @if($server->swap === 0)
                                        <code data-toggle="tooltip" data-placement="top" title="Swap Space">Не задан</code>
                                    @elseif($server->swap === -1)
                                        <code data-toggle="tooltip" data-placement="top" title="Swap Space">Без ограничений</code>
                                    @else
                            ...
                            <tr>
                                <td>Подключение по умолчанию</td>
                    ...
                    @if(!$server->isInstalled())
                        <div class="col-sm-12">
                            <div class="small-box {{ (! $server->isInstalled()) ? 'bg-blue' : 'bg-maroon' }}">
                                <div class="inner">
                                    <h3 class="no-margin">{{ (! $server->isInstalled()) ? 'Устанавливается' : 'Ошибка установки' }}</h3>
                                </div>
                            </div>
                        </div>
                    @endif
