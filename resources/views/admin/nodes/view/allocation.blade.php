...
@section('content-header')
    <h1>{{ $node->name }}<small>Управление портами, доступными для серверов на этой ноде.</small></h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('admin.index') }}">Админ</a></li>
        <li><a href="{{ route('admin.nodes') }}">Ноды</a></li>
        <li><a href="{{ route('admin.nodes.view', $node->id) }}">{{ $node->name }}</a></li>
        <li class="active">Порты</li>
    </ol>
@endsection
    ...
    <div class="col-xs-12">
        <div class="nav-tabs-custom nav-tabs-floating">
            <ul class="nav nav-tabs">
                ...
                <li class="active"><a href="{{ route('admin.nodes.view.allocation', $node->id) }}">Порты</a></li>
    ...
    <div class="col-sm-8">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">Существующие порты</h3>
            </div>
    ...
    <div class="col-sm-4">
        <form action="{{ route('admin.nodes.view.allocation', $node->id) }}" method="POST">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">Назначить новые порты</h3>
                </div>
                <div class="box-body">
                    <div class="form-group">
                        ...
                        <div>
                            ...
                            <p class="text-muted small">Введите IP-адрес, для которого нужно назначить порты.</p>
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="pAllocationIP" class="control-label">Псевдоним IP</label>
                        <div>
                            <input type="text" id="pAllocationAlias" class="form-control" name="allocation_alias" placeholder="alias" />
                            <p class="text-muted small">Если хотите назначить этим портам псевдоним по умолчанию, укажите его здесь.</p>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div>
                            ...
                            <p class="text-muted small">Введите отдельные порты или диапазоны портов через запятую или пробел.</p>
                        </div>
                    </div>
                </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Удалить порты для блока IP</h4>
            </div>
            <form action="{{ route('admin.nodes.view.allocation.removeBlock', $node->id) }}" method="POST">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-danger">Удалить порты</button>
                </div>
            </form>
        </div>
    </div>
...
@section('footer-scripts')
    ...
    $('button[data-action="deallocate"]').click(function (event) {
        ...
        swal({
            title: '',
            text: 'Вы уверены, что хотите удалить этот порт?',
            type: 'warning',
            ...
            $.ajax({
                ...
                swal({ type: 'success', title: 'Порт удалён!' });
            }).fail(function (jqXHR) {
                console.error(jqXHR);
                swal({
                    title: 'Упс!',
    ...
    function deleteSelected() {
        ...
        if (selectedItems.length !== 0) {
            ...
            if (selectedItems.length > 5) {
                formattedItems += ', и ещё ' + (selectedItems.length - 5) + ' шт.';
            }

            swal({
                type: 'warning',
                title: '',
                text: 'Вы уверены, что хотите удалить следующие порты: ' + formattedItems + '?',
                html: true,
                showCancelButton: true,
                ...
                $.ajax({
                    ...
                    swal({
                        ...
                        title: 'Упс!',
                        html: true,
                        text: 'Произошла ошибка при попытке удалить эти порты. Попробуйте снова.',
                    });
                });
            });
        } else {
            swal({
                type: 'warning',
                title: '',
                text: 'Пожалуйста, выберите порты для удаления.',
            });
        }
    }
    </script>
