...
@section('content-header')
    <h1>{{ $node->name }}<small>Ваш конфигурационный файл daemon.</small></h1>
    <ol class="breadcrumb">
    ...
    <div class="col-sm-8">
        <div class="box box-primary">
            ...
            <div class="box-footer">
                <p class="no-margin">Этот файл нужно поместить в корневой каталог daemon (обычно <code>/etc/pterodactyl</code>) в файл с именем <code>config.yml</code>.</p>
            </div>
        </div>
    </div>
    <div class="col-sm-4">
        <div class="box box-success">
            ...
            <div class="box-body">
                <p class="text-muted small">
                    Используйте кнопку ниже, чтобы сгенерировать специальную команду развёртывания,
                    которую можно использовать для настройки wings на целевом сервере одной командой.
                </p>
            </div>
            <div class="box-footer">
    ...
    $('#configTokenBtn').on('click', function (event) {
        $.ajax({
            ...
            swal({
                type: 'success',
                title: 'Токен создан.',
                text: '<p>Чтобы автоматически настроить вашу ноду, выполните следующую команду:<br /><small><pre>cd /etc/pterodactyl && sudo wings configure --panel-url {{ config('app.url') }} --token ' + data.token + ' --node ' + data.node + '{{ config('app.debug') ? ' --allow-insecure' : '' }}</pre></small></p>',
                html: true
            })
        }).fail(function () {
            swal({
