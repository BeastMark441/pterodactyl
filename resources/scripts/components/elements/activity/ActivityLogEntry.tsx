            ...
            <div className={'col-span-10 sm:col-span-9 flex'}>
                <div className={'flex-1 px-4 sm:px-0'}>
                    <div className={'flex items-center text-gray-50'}>
                        <Tooltip placement={'top'} content={actor?.email || 'Системный пользователь'}>
                            <span>{actor?.username || 'Система'}</span>
                        </Tooltip>
                        ...
                        <div className={classNames(style.icons, 'group-hover:text-gray-300')}>
                            {activity.isApi && (
                                <Tooltip placement={'top'} content={'Использование ключа API'}>
                                    <TerminalIcon />
                                </Tooltip>
                            )}
                            {activity.event.startsWith('server:sftp.') && (
                                <Tooltip placement={'top'} content={'Использование SFTP'}>
                                    <FolderOpenIcon />
                                </Tooltip>
                            )}
                            {children}
                    ...
                    <div className={'mt-1 flex items-center text-sm'}>
                        ...
                        <Tooltip placement={'right'} content={format(activity.timestamp, 'dd.MM.yyyy, HH:mm:ss', { locale: ru })}>
                            <span>{formatDistanceToNowStrict(activity.timestamp, { addSuffix: true, locale: ru })}</span>
                        </Tooltip>
                    </div>
                </div>
