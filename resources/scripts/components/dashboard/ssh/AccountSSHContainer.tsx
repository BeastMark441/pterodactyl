...
export default () => {
    ...
    return (
        <PageContentBlock title={'SSH Ключи'}>
            <FlashMessageRender byKey={'account'} />
            <div css={tw`md:flex flex-nowrap my-10`}>
                <ContentBox title={'Добавить SSH-ключ'} css={tw`flex-none w-full md:w-1/2`}>
                    <CreateSSHKeyForm />
                </ContentBox>
                <ContentBox title={'SSH Ключи'} css={tw`flex-1 overflow-hidden mt-8 md:mt-0 md:ml-8`}>
                    <SpinnerOverlay visible={!data && isValidating} />
                    {!data || !data.length ? (
                        <p css={tw`text-center text-sm`}>
                            {!data ? 'Загрузка...' : 'Для этой учетной записи не существует SSH-ключей.'}
                        </p>
                    ) : (
                        data.map((key, index) => (
                            <GreyRowBox
