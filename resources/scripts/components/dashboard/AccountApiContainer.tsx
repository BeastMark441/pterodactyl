    ...
    return (
        <PageContentBlock title={'API Аккаунта'}>
            ...
            <div css={tw`md:flex flex-nowrap my-10`}>
                <ContentBox title={'Создать API Ключ'} css={tw`flex-none w-full md:w-1/2`}>
                    <CreateApiKeyForm onKeyCreated={(key) => setKeys((s) => [...s!, key])} />
                </ContentBox>
                <ContentBox title={'API Ключи'} css={tw`flex-1 overflow-hidden mt-8 md:mt-0 md:ml-8`}>
                    ...
                    >
                        Все запросы, использующие ключ <Code>{deleteIdentifier}</Code>, будут аннулированы.
                    </Dialog.Confirm>
                    {keys.length === 0 ? (
                        <p css={tw`text-center text-sm`}>
                            {loading ? 'Загрузка...' : 'Для этого аккаунта не существует API ключей.'}
                        </p>
                    ) : (
                        keys.map((key, index) => (
                            <GreyRowBox
