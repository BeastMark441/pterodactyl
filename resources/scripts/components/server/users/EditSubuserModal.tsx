    ...
    return (
        <Formik
            ...
            validationSchema={object().shape({
                email: string()
                    .max(191, 'Email адрес не должен превышать 191 символ.')
            ...
            <Form>
                <div css={tw`flex justify-between`}>
                    <h2 css={tw`text-2xl`} ref={ref}>
                        {subuser
                            ? `${canEditUser ? 'Изменить' : 'Просмотр'} права для ${subuser.email}`
                            : 'Создать нового пользователя'}
                    </h2>
                    <div>
                        <Button type={'submit'} css={tw`w-full sm:w-auto`}>
                            {subuser ? 'Сохранить' : 'Добавить Пользователя'}
                        </Button>
                    </div>
                </div>
                ...
                {!isRootAdmin && loggedInPermissions[0] !== '*' && (
                    <div css={tw`mt-4 pl-4 py-2 border-l-4 border-cyan-400`}>
                        <p css={tw`text-sm text-neutral-300`}>
                            При создании или изменении других пользователей можно выбрать только те разрешения, 
                            которые назначены вашему аккаунту.
                        </p>
                    </div>
                )}
                {!subuser && (
                    <div css={tw`mt-6`}>
                        <Field
                            ...
                            description={
                                'Введите email адрес пользователя, которого вы хотите пригласить как дополнительного пользователя для этого сервера.'
                            }
                        />
                    </div>
                )}
