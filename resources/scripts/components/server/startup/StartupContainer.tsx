...
const StartupContainer = () => {
    ...
    return !data ? (
        ...
        <ServerContentBlock title={'Параметры запуска'} showFlashKey={'startup:image'}>
            <div css={tw`md:flex`}>
                ...
                <TitledGreyBox title={'Образ Docker'} css={tw`flex-1 lg:flex-none lg:w-1/3 mt-8 md:mt-0 md:ml-10`}>
                    {Object.keys(data.dockerImages).length > 1 && !isCustomImage ? (
                        ...
                        <>
                            ...
                            {isCustomImage && (
                                <p css={tw`text-xs text-neutral-300 mt-2`}>
                                    Этот серверный образ Docker был установлен администратором вручную и не может
                                    может быть изменен с помощью этого пользовательского интерфейса.
                                </p>
                            )}
                        </>
                    )}
