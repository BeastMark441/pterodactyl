    ...
    const submit = ({ name }: FormikValues, { setSubmitting }: FormikHelpers<FormikValues>) => {
        ...
        if (files.length === 1) {
            if (!useMoveTerminology && len === 1) {
                // Переименовать файл в этом каталоге.
                mutate((data) => data.map((f) => (f.name === files[0] ? { ...f, name } : f)), false);
            } else if (useMoveTerminology || len > 1) {
                // Удалить файл из этого каталога, так как он был перемещен в другое место.
                        ...
                        <div css={[tw`flex flex-wrap`, useMoveTerminology ? tw`items-center` : tw`items-end`]}>
                            <div css={tw`w-full sm:flex-1 sm:mr-4`}>
                                <Field
                                    ...
                                    description={
                                        useMoveTerminology
                                            ? 'Введите новое имя и каталог этого файла или папки относительно текущего каталога.'
                                            : undefined
                                    }
                                    autoFocus
                                />
                            ...
                            <div css={tw`w-full sm:w-auto mt-4 sm:mt-0`}>
                                <Button css={tw`w-full`}>{useMoveTerminology ? 'Переместить' : 'Переименовать'}</Button>
                            </div>
                        </div>
                        {useMoveTerminology && (
                            <p css={tw`text-xs mt-2 text-neutral-400`}>
                                <strong css={tw`text-neutral-200`}>Новое местоположение:</strong>
