...
const schema = object().shape({
    directoryName: string().required('Необходимо указать действительное имя каталога.'),
});
        ...
        <Formik onSubmit={submit} validationSchema={schema} initialValues={{ directoryName: '' }}>
            {({ submitForm, values }) => (
                <>
                    ...
                    <Form css={tw`m-0`}>
                        <Field autoFocus id={'directoryName'} name={'directoryName'} label={'Имя каталога'} />
                        <p css={tw`mt-2 text-sm md:text-base break-all`}>
                            <span css={tw`text-neutral-200`}>Этот каталог будет создан следующим образом&nbsp;</span>
                            <Code>
                                /home/container/
