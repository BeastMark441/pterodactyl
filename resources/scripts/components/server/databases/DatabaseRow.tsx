...
export default ({ database, className }: Props) => {
    ...
    const schema = object().shape({
        confirm: string()
            .required('Имя базы данных должно быть указано.')
            .oneOf([database.name.split('_', 2)[1], database.name], 'Имя базы данных должно быть указано.'),
    });
    ...
    return (
        <>
            <Formik onSubmit={submit} initialValues={{ confirm: '' }} validationSchema={schema} isInitialValid={false}>
                {({ isSubmitting, isValid, resetForm }) => (
                    ...
                    >
                        ...
                        <p css={tw`text-sm`}>
                            Удаление базы данных является необратимым действием, оно не может быть отменено. Это навсегда
                            удалит базу данных <strong>{database.name}</strong> и удалит все связанные данные.
                        </p>
            ...
            <Modal visible={connectionVisible} onDismissed={() => setConnectionVisible(false)}>
                ...
                <div css={tw`mt-6`}>
                    <Label>JDBC строка подключения</Label>
            ...
            <GreyRowBox $hoverable={false} className={className} css={tw`mb-2`}>
                ...
                <div css={tw`ml-8 text-center hidden md:block`}>
                    ...
                    <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`}>Подключения из</p>
                </div>
                <div css={tw`ml-8 text-center hidden md:block`}>
                    <CopyOnClick text={database.username}>
