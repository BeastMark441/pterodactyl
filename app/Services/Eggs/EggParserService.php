...
class EggParserService
{
    ...
    public function handle(UploadedFile $file): array
    {
        if ($file->getError() !== UPLOAD_ERR_OK || !$file->isFile()) {
            throw new InvalidFileUploadException('Выбранный файл недействителен и не может быть импортирован.');
        }
        ...
        if (!in_array(Arr::get($parsed, 'meta.version') ?? '', ['PTDL_v1', 'PTDL_v2'])) {
            throw new InvalidFileUploadException('Предоставленный файл JSON не имеет такого формата, который можно было бы распознать.');
        }
