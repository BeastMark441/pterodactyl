...
class DatabaseManagementService
{
    ...
    public function create(Server $server, array $data): Database
    {
        ...
        if (empty($data['database']) || !preg_match(self::MATCH_NAME_REGEX, $data['database'])) {
            throw new \InvalidArgumentException('Имя базы данных, переданное в DatabaseManagementService::handle ДОЛЖНО иметь префикс "s{server_id}_".');
        }
    ...
    protected function createModel(array $data): Database
    {
        ...
        if ($exists) {
            throw new DuplicateDatabaseNameException('База данных с таким именем уже существует для этого сервера.');
        }
